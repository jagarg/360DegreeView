...
*/
      ...
      function getDisplayTable(d){
        ...
        var displayTable =  
              ...
              +'</tr>'
              +'</thead>'
              +'<tbody id="'+ tableName +'" class="collapse">';
...
*/
    ...
    function getTargetColumn(d) {
      return d['targetColumn'];
    }
        ...
        if (data instanceof Array) {
          ...
          if (data.vertices) {
            data.vertices.forEach(function (elem) {
              ...
              if (elem["type"]) {
                if (self.classesInCanvas.vertices.indexOf(elem["type"]) == -1) {
                  self.classesInCanvas.vertices.push(elem["type"]);
                }
              }
            })
          }
    ...
    dataArray: function (data) {
      ...
      data.forEach(function (elem) {
          ...
          if (self.isVertex(elem)) {
            ...
            if (elem["type"]) {
              if (self.classesInCanvas.indexOf(elem["type"]) == -1) {
                self.classesInCanvas.push(elem["type"]);
              }
            }
            var v = self.get(elem['rid']);
            if (!v) {
            ...
            var e = new OEdge(self, v2, v1, elem['type'], elem);
            self.addEdge(e);
          }
